...
CUDA-graph replay per step, rvLLM~v3 reaches \textbf{22{,}496 tokens/second
at $N\!=\!128$} (decode + LM head + argmax, 30 iterations, 5 warmup). For
direct comparison, vLLM~0.19 (V1 engine, FP8, FlashInfer, CUDA graphs on)
running on the same GPU with the same model and batch size reports
\textbf{19{,}399 tok/s} as its steady-state generation throughput; rvLLM~v3
is \textbf{1.16}$\boldsymbol\times$ \textbf{faster} at the kernel level.
The earlier v2 stack peaked at 19{,}287 tok/s at the same batch
...
This paper documents rvLLM~v3, which achieves 22{,}496~tok/s at
...
\texttt{eb9e247fd}, April~16, 2026) reached 19{,}287~tok/s; vLLM~0.19 V1
on the same box measures 19{,}399~tok/s. v3 is 1.16$\times$ faster than
both on the same GPU with the same model, same FP8 quantization, same
CUDA-graph configuration. v3's primary levers are: (i)
...
describes the v3 session (551\,\ensuremath{\rightarrow}\,22{,}496 tok/s).
...
32  & 6{,}644 & 4.82 \\
64  & 12{,}615 & 5.07 \\
128 & \textbf{22{,}496} & \textbf{5.67} \\
\bottomrule
\end{tabular}

\subsection{Head-to-head at $N\!=\!128$: v3 vs vLLM vs v2 archive}

\begin{tabular}{lrr}
\toprule
\textbf{Engine} & \textbf{tok/s} & \textbf{ratio to v3} \\
\midrule
vLLM 0.19 V1 (FP8, FlashInfer, CUDA graphs) & 19{,}399 & 0.862$\times$ \\
rvllm-v2 (archive, \texttt{eb9e247fd}) & 19{,}287 & 0.858$\times$ \\
\textbf{rvllm-v3 (fair bench)} & \textbf{22{,}496} & \textbf{1.00}$\boldsymbol\times$ \\
\bottomrule
\end{tabular}
...
After all three fixes, v3 measures \textbf{22{,}496~tok/s} (vs.\ 22{,}559
on the frozen-graph version), a 0.3\% decrease. The per-step metadata
upload and faux-prefill are small relative to the 28-layer forward pass
and LM head. v3 remains 16\% faster than vLLM, confirming the edge is
structural (fused QKV, cuBLASLt autotune, fused epilogues, no Python
dispatch) rather than an artifact of benchmark setup.
...
\textbf{+ all 5 linears on cuBLASLt}          & \textbf{22{,}496} & \textbf{+28\%} \\
\bottomrule
\end{tabular}
...
\textbf{22{,}496~tok/s} serving Qwen2.5-7B in FP8 on a single H100, with
