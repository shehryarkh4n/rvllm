// Numerically stable softmax for f16 input.
// Reads f16 logits, computes in f32, writes f32 output (for sampling/argmax).
//
// Launch config:
//   Grid:  (num_rows, 1, 1)
...
__global__ void softmax_f16_kernel(
    ...
    const __half* x = input + row * vocab_size;
    ...
    for (int i = tid; i < vocab_size; i += stride) {
        float e = expf(__half2float(x[i]) - row_max);
        y[i] = e;
        local_sum += e;
    }
    smem[tid] = local_sum;
